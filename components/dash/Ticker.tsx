...
export async function Ticker() {
  ...
  if (week) {
    ...
    if (sysOn("leader")) {
      ...
      // The weekend projection decides who leads (D-073, D-075) — otherwise the rail
      // crowns whoever risked the least between the reveal and settlement.
      const [{ data: rows }, proj] = await Promise.all([
        db.from("standings").select("player_id, first_name, last_name, stack, status"),
        loadProjection(db),
      ]);
      const state = leaderFrom(withProjection(rows ?? [], proj));
