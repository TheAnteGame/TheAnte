...
describe("kickoffs and week anchors (kickoffs Eastern, anchors Mountain, DST-safe — ANTE-TECH §4.5)", () => {
  ...
  it("Week 1 2026: Wednesday opener anchors to Tue Sep 8 6:00am MT / Thu Sep 10 noon MT", () => {
    const { opensAt, deadlineAt } = weekAnchors(kickoffFromNflverse("2026-09-09", "20:20"));
    const open = DateTime.fromJSDate(opensAt).setZone("America/Denver");
    const dead = DateTime.fromJSDate(deadlineAt).setZone("America/Denver");
  ...
  it("Week 10 spans the November DST change and the deadline stays at noon MT", () => {
    // DST ends Sun Nov 1 2026. A Thursday Nov 5 game week: anchors must be clean MT.
    ...
    expect(dead.offset).toBe(-7 * 60); // MST after the change — tz database, not offsets
  });
